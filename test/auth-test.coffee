...
describe "auth", ->
  ...
  context "<user> has <role> role", ->
    ...
    it "admin user successfully sets role for name with whitespace", ->
  ...
  context "<user name> doesn't have <role> role", ->

    it "admin user successfully removes role from user name containing space", ->
      @room.user.say("alice", "hubot: jimmy jones has demo role")
